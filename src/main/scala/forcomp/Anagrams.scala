...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    if (occurrences.isEmpty) List(List())
      ...
      val (k, v) = occurrences.head
      for (n <- (0 to v).toList; others <- combinations(occurrences.tail))
        yield {
          if (n == 0) others
          else (k, n) :: others
        }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    ...
    ((x foldLeft List[(Char, Int)]()) { case (r, (k, v)) => List((k, v - yMap.apply(k))) ::: r } filterNot { case (k, v) => v == 0 }).sorted
  }
    ...
    def occurrenceAnagrams(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty) List(List())
        ...
        for {
          ...
          rest <- occurrenceAnagrams(subtract(occurrences, comb))
